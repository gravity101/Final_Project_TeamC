...
class Xycar(object):
    ...
    def control(self) :
        ...
        if self.obstacle.obstacle_state == "left turn" and 30000 < self.stanley_follower.index < 57000 :
            self.msg.angle = -50
        elif self.obstacle.obstacle_state == "right turn" and 30000 < self.stanley_follower.index < 57000 :
            self.msg.angle = 50
        ...
        """        
        if self.pparking.forward == 2:
            for i in range(445, 505):
                print("index: ", i, " lidar: ", self.pparking.lidar_points[i])
            time.sleep(100)
        """

        if self.pparking.front_flag and self.pparking.right_flag and self.stanley_follower.index > 57000:
            print("Over")
            self.msg.angle = 0
            self.msg.speed = 0
            self.pub.publish(self.msg)
        ...
        if (self.pparking.sonic_flag or self.pparking.right_flag) and not self.pparking.front_flag and not self.pparking.over_flag and self.stanley_follower.index > 57000:
            print("앞으로")
